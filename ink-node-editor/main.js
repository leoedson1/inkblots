...
function buildMenu() {
  // All menus are rendered in the app, including Ink hover descriptions.
...
ipcMain.on('window-theme', (event, theme) => {
  ...
  win.setTitleBarOverlay(theme === 'light'
    ? { color: '#f7f8fa', symbolColor: '#202633', height: 48 }
    : { color: '#161a23', symbolColor: '#e3e7f0', height: 48 });
});
ipcMain.on('window-action', (event, action) => {
  if (!win || event.sender !== win.webContents) return;
  const wc = win.webContents;
  const actions = {
    undo: () => wc.undo(), redo: () => wc.redo(), cut: () => wc.cut(),
    copy: () => wc.copy(), paste: () => wc.paste(), selectAll: () => wc.selectAll(),
    minimize: () => win.minimize(), maximize: () => win.isMaximized() ? win.unmaximize() : win.maximize(),
    fullscreen: () => win.setFullScreen(!win.isFullScreen()),
    reload: () => wc.reload(), devtools: () => wc.toggleDevTools(),
    zoomIn: () => wc.setZoomLevel(wc.getZoomLevel() + 1),
    zoomOut: () => wc.setZoomLevel(wc.getZoomLevel() - 1), resetZoom: () => wc.setZoomLevel(0),
    help: () => shell.openExternal('https://github.com/inkle/ink/blob/master/Documentation/WritingWithInk.md'),
  };
